module.exports = async function handler(req, res) {
  // 1. Enable CORS so your AnimeVault frontend can fetch this
  ...
  // 2. Get the slug and episode from the URL (Vercel parses [slug] and [episode] automatically)
  const { slug, episode } = req.query;

  if (!slug || !episode) {
    return res.status(400).json({ error: 'Missing slug or episode in URL' });
  }
  ...
  try {
    ...
    // 5. Helper function to extract MP4 sources from the embed page
    const extractSources = async (embedPath, type) => {
      ...
      // Find the videoSources array in the script
    ...
    // 7. Return the clean JSON to your AnimeVault frontend
